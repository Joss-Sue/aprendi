...
drop procedure login_usuario; --porque está esto aquí?
...
DELIMITER ; -- A ESTE PROCEDURE LE FALTA ALGO
...
CREATE PROCEDURE sp_ventas_por_curso(IN curso_id_param INT) -- Procedure duplicado
BEGIN
    SELECT 
        u.nombre AS nombre_estudiante,
        c.titulo AS curso_titulo,
...
------------------------------------------------------------------------------------------------------
-- Triggers, vistas y funciones faltantes
------------------------------------------------------------------------------------------------------
--------------------------- Triggers -----------------------------

DELIMITER //

CREATE TRIGGER bloquear_usuario_tras_intentos_fallidos -- Bloquea un usuario si ha tenido más de 3 intentos fallidos de inicio de sesión.
AFTER UPDATE ON Usuarios
FOR EACH ROW
BEGIN
    IF NEW.intentos_fallidos > 3 THEN
        UPDATE Usuarios
        SET estado = 3 -- Estado 3: Bloqueado
        WHERE id = NEW.id;
    END IF;
END;
//
...
CREATE TRIGGER desactivar_niveles_con_curso -- Al cambiar el estado de un curso a 0, todos sus niveles se desactivan
...
CREATE FUNCTION promedio_progreso_curso(id_curso INT) -- Devuelve el promedio del progreso de los estudiantes en un curso específico
RETURNS DECIMAL(5, 2)
DETERMINISTIC
BEGIN
    DECLARE promedio DECIMAL(5, 2);
    SELECT AVG(progreso_curso)
    INTO promedio
    FROM Inscripciones
...
CREATE FUNCTION contar_mensajes_activos(id_usuario INT) -- regresad la cantidad de mensajes activos enviados por un usuario.
RETURNS INT
DETERMINISTIC
BEGIN
    ...
    INTO total_mensajes
    FROM Mensajes
    WHERE remitente_id = id_usuario AND estado = 1;
...
-- -------------- vistas -------------------------
CREATE VIEW vista_progreso_estudiantes AS -- Vista para listar estudiantes y su progreso en cada curso
SELECT 
    I.estudiante_id,
    U.nombre AS nombre_estudiante,
    I.curso_id,
...
CREATE VIEW vista_comentarios_cursos AS -- Muestra los comentarios activos junto con la calificación y el nombre del curso.
SELECT 
    Co.curso_id,
    ...
    Co.fecha
FROM Comentarios Co
JOIN Cursos Cu ON Co.curso_id = Cu.id
...
SELECT 
    ...
    Cu.estado
FROM Cursos Cu
JOIN Categorias Ca ON Cu.categoria_id = Ca.id
JOIN Usuarios U ON Cu.instructor_id = U.id;

-- Prueba Funciones
SELECT promedio_progreso_curso(1); -- Reemplaza "1" con un id_curso válido.
SELECT contar_mensajes_activos(2); -- Reemplaza "2" con un id_usuario válido.

-- Prueba Vistas
SELECT * FROM vista_progreso_estudiantes;
SELECT * FROM vista_comentarios_cursos;
SELECT * FROM vista_cursos_por_categoria;
